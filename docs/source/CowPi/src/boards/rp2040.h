...
typedef struct {
    ...
    uint32_t peripheral_address;        //!< I2C peripheral address register (IC_SAR)
    uint32_t :32;                       //!< padding (apparently unusued address)
    uint32_t data:12;                   //!< I2C data buffer and command register (IC_DATA_CMD)
    uint32_t :20;                       //!< padding (unused bits 31..12 of IC_DATA_CMD)
